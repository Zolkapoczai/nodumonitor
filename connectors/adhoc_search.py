...
_DEFAULT_CHANNELS = ["reddit", "stackoverflow"]


def run_adhoc_search(config: dict, db_path: str, query: str, channels: list[str] = None) -> dict:
    """
    Lefuttatja az ad-hoc keresest a valasztott csatornakon.
    Visszaad: {"query": ..., "channels": {csatorna: talalat-szam}, "total": ...}.
    """
    ...
    print(f"[adhoc] '{query}': {total} talalat ({per_channel})")
