...
19. **⚠️ Az admin „Mentés" gombja LETÖRLI a config.yaml MINDEN kommentjét.** A `ui/app.py → save_config()` `yaml.dump`-pal írja ki a fájlt, a PyYAML pedig nem tartja meg a kommenteket. A config.yaml-ben viszont ma **működési szabályok** vannak kommentként (miért `updated_at` a Zendesknél, miért `'off'` idézőjelben, miért nincs itt a webhook-URL, miért ez a két Zendesk-szekció) — egyetlen admin-mentés az összeset elviszi, és a következő session újra feltalálja a rossz megoldást. **Amíg nincs javítva** (kommentőrző író kellene, pl. `ruamel.yaml` — új függőség, külön feladat): a config.yaml-t **kézzel** szerkeszd, és admin-mentés után ellenőrizd a diffet. Régi (nem a Slack-munkával keletkezett) hiba, de 2026-07-27 óta több szabály is kommentben él, tehát a kockázat nőtt.
